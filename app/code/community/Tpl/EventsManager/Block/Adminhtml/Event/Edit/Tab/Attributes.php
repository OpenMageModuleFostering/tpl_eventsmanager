 ...
 */
/**
 * Event admin edit tab attributes block
 * @category    Tpl
 * @package     Tpl_EventsManager
 * @author      TPL
*/
class Tpl_EventsManager_Block_Adminhtml_Event_Edit_Tab_Attributes extends Mage_Adminhtml_Block_Widget_Form
{
    /**
    ...
    protected function _prepareForm()
    {
        ...
        $form->setDataObject(Mage::registry('current_event'));
        $fieldset = $form->addFieldset(
            'info',
            array(
                'legend' => Mage::helper('tpl_eventsmanager')->__('Event Information'),
                'class' => 'fieldset-wide',
            )
        );
        $attributes = $this->getAttributes();
        foreach ($attributes as $attribute) {
            $attribute->setEntity(Mage::getResourceModel('tpl_eventsmanager/event'));
        }
        $this->_setFieldset($attributes, $fieldset, array());
        $formValues = Mage::registry('current_event')->getData();
    ...
    protected function _getAdditionalElementTypes()
    {
        return array(
            'file'     => Mage::getConfig()->getBlockClassName(
                'tpl_eventsmanager/adminhtml_event_helper_file'
            ),
            'image'    => Mage::getConfig()->getBlockClassName(
                'tpl_eventsmanager/adminhtml_event_helper_image'
            ),
