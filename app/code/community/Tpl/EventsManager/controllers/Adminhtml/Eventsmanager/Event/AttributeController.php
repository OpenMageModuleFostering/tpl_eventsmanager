 ...
 */
/**
 * Event admin attribute controller
 *
 * @category    Tpl
 * @package     Tpl_EventsManager
 * @author      TPL
 */
class Tpl_EventsManager_Adminhtml_EventsManager_Event_AttributeController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function preDispatch()
    {
        ...
        $this->_entityTypeId = Mage::getModel('eav/entity')
            ->setType(Tpl_EventsManager_Model_Event::ENTITY)
            ->getTypeId();
    }
     ...
     *
     * @accees protected
     * @return Tpl_EventsManager_Adminhtml_Event_AttributeController
     * @author TPL
     */
    protected function _initAction()
    {
        $this->_title(Mage::helper('tpl_eventsmanager')->__('Event'))
             ->_title(Mage::helper('tpl_eventsmanager')->__('Attributes'))
             ->_title(Mage::helper('tpl_eventsmanager')->__('Manage Attributes'));

        $this->loadLayout()
            ->_setActiveMenu('tpl_eventsmanager/event_attributes')
            ->_addBreadcrumb(
                Mage::helper('tpl_eventsmanager')->__('Event'),
                Mage::helper('tpl_eventsmanager')->__('Event')
            )
            ->_addBreadcrumb(
                Mage::helper('tpl_eventsmanager')->__('Manage Event Attributes'),
                Mage::helper('tpl_eventsmanager')->__('Manage Event Attributes')
            );
        return $this;
    }
    ...
    public function editAction()
    {
        ...
        if ($id) {
            ...
            if (! $model->getId()) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('tpl_eventsmanager')->__('This event attribute no longer exists')
                );
                $this->_redirect('*/*/');
                return;
            }
            ...
            if ($model->getEntityTypeId() != $this->_entityTypeId) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('tpl_eventsmanager')->__('This event attribute cannot be edited.')
                );
                $this->_redirect('*/*/');
                return;
            }
        ...
        $this->_initAction();
        $this->_title($id ? $model->getName() : Mage::helper('tpl_eventsmanager')->__('New Event Attribute'));
        $item = $id ? Mage::helper('tpl_eventsmanager')->__('Edit Event Attribute')
                    : Mage::helper('tpl_eventsmanager')->__('New Event Attribute');
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            $model        = Mage::getModel('tpl_eventsmanager/resource_eav_attribute');
            $helper       = Mage::helper('tpl_eventsmanager/event');
            $id           = $this->getRequest()->getParam('attribute_id');
            ...
            try {
                $model->save();
                $session->addSuccess(
                    Mage::helper('tpl_eventsmanager')->__('The event attribute has been saved.')
                );
                /**
        ...
        if ($id = $this->getRequest()->getParam('attribute_id')) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('tpl_eventsmanager')->__('The event attribute has been deleted.')
                );
                $this->_redirect('*/*/');
                return;
            }
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('tpl_eventsmanager/event_attributes');
    }
}
