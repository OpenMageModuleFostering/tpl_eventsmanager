					...
					$(d).find('event').each(function(){
						...
						var fullstartDate = $(this).find('startdate').text(),
							...
							eventTitle = $(this).find('name').text(),
                                                        eventDescription = $(this).find('description').text()
						...
						function multidaylist(){
							$('#'+uniqueId+' .monthly-list-item[data-number="'+i+'"]').addClass('item-has-event').append('<a href="'+eventURL+'" class="listed-event"  data-eventid="'+ eventId +'" style="background:'+eventColor+'" title="'+eventTitle+'">'+eventTitle+'<div><div style="width:100%; text-align: right;  margin-bottom: 3px;"><div class="monthly-list-time-start">'+startDay+'/'+startMonth+'/'+ startYear+'  </div><div class="monthly-list-time-end">  '+endDay+'/'+endMonth+'/'+ endYear+'</div></div><div style="color:white;height: 66px; display: block; overflow:hidden">'+eventDescription+'</div></div></a>');
						}
		...
		$(document.body).on('click', '#'+uniqueId+' a.monthly-day', function (e) {
			...
			if(options.mode == 'event' && options.eventList == true) {
				...
				var myElement = document.getElementById(uniqueId+'day'+whichDay);
                               // $('#' + uniqueId+'day'+whichDay).css('height','97%');
